if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("LUQ INFINITE GT is already running!", 0)
    return
end
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "LUQ INFINITE GT is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "LUQ INFINITE GT FE v" .. currentVersion
...
reference = (function()
	local main = create({
		...
		{116,"TextButton",{BackgroundColor3=Color3.new(0.48627451062202,0.61960786581039,0.85098040103912),BorderColor3=Color3.new(0.1803921610117,0.1803921610117,0.1843137294054),Font=4,Name="InviteButton",Parent={112},Position=UDim2.new(0,5,0,75),Size=UDim2.new(1,-10,0,25),Text="Copy Discord Invite Link (
	})
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- LUQ INFINITE GT Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the LUQ INFINITE GT support server.'}
...
addcmd("serverhop", {"shop"}, function(args, speaker)
    ...
    if httprequest then
        local servers = {}
        local req = httprequest({Url = string.format(" PlaceId)})
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, LUQ INFINITE GT may have issues or features that are not functioning correctly.") end
end)


-- Novos Comandos Integrados

-- Registrar novos comandos no sistema existente

-- Função: noclip
addcmd('noclip', {}, 'Ativa/desativa o modo Noclip (atravessar objetos).', function()
...
-- Função: mensagem global (ms)
addcmd('ms', {'mensagem'}, 'Envia uma mensagem global para todos os jogadores.', function(args)
    ...
    for _, player in ipairs(game.Players:GetPlayers()) do
        ...
        label.TextColor3 = Color3.new(1, 1, 1)
        label.BackgroundTransparency = 1
        game:GetService("Debris"):AddItem(gui, 5) -- Mensagem dura 5 segundos
    end
    notify('Mensagem enviada para todos.')
end)

-- Função: transformar em gato (cat)
addcmd('cat', {}, 'Transforma seu personagem em um gato que pode ser montado.', function()
    local player = game.Players.LocalPlayer
    local character = player.Character or player.CharacterAdded:Wait()
    local catModelId = "rbxassetid://1234567890" -- Substituir por um ID real
    local catModel = game:GetService("InsertService"):LoadAsset(catModelId):GetChildren()[1]
    catModel.Parent = character
    local seat = Instance.new("VehicleSeat", catModel)
    seat.Name = "RideSeat"
    notify('Você se transformou em um gato!')
end)

-- Função: fly
addcmd('fly', {}, 'Ativa/desativa o modo Fly.', function()
    local flying = false
    local player = game.Players.LocalPlayer
    local character = player.Character or player.CharacterAdded:Wait()
    ...
    if flying then
        ...
        notify('Modo Fly ativado.')
    else
        humanoid.PlatformStand = false
        humanoid.JumpPower = 50
        humanoid.WalkSpeed = 16
        notify('Modo Fly desativado.')
    end
end)

-- Placeholder for additional admin commands
addCommand('command1', function(args) notify("Command1", "Executed command1!", 5) end)
addCommand('command2', function(args) notify("Command2", "Executed command2!", 5) end)
addCommand('command3', function(args) notify("Command3", "Executed command3!", 5) end)
addCommand('command4', function(args) notify("Command4", "Executed command4!", 5) end)
addCommand('command5', function(args) notify("Command5", "Executed command5!", 5) end)
addCommand('command6', function(args) notify("Command6", "Executed command6!", 5) end)
addCommand('command7', function(args) notify("Command7", "Executed command7!", 5) end)
addCommand('command8', function(args) notify("Command8", "Executed command8!", 5) end)
addCommand('command9', function(args) notify("Command9", "Executed command9!", 5) end)
addCommand('command10', function(args) notify("Command10", "Executed command10!", 5) end)
addCommand('command11', function(args) notify("Command11", "Executed command11!", 5) end)
addCommand('command12', function(args) notify("Command12", "Executed command12!", 5) end)
addCommand('command13', function(args) notify("Command13", "Executed command13!", 5) end)
addCommand('command14', function(args) notify("Command14", "Executed command14!", 5) end)
addCommand('command15', function(args) notify("Command15", "Executed command15!", 5) end)
addCommand('command16', function(args) notify("Command16", "Executed command16!", 5) end)
addCommand('command17', function(args) notify("Command17", "Executed command17!", 5) end)
addCommand('command18', function(args) notify("Command18", "Executed command18!", 5) end)
addCommand('command19', function(args) notify("Command19", "Executed command19!", 5) end)
addCommand('command20', function(args) notify("Command20", "Executed command20!", 5) end)
...
addCommand('speed', function(args)
    ...
    if humanoid and args[1] then
        ...
        notify("Speed", "Speed set to " .. humanoid.WalkSpeed, 5)
    end
end)

-- Size Command
addCommand('size', function(args)
    local player = game.Players.LocalPlayer
    local character = player.Character or player.CharacterAdded:Wait()
    local humanoid = character:FindFirstChildOfClass("Humanoid")
    if humanoid and args[1] then
        ...
        notify("Size", "Size set to " .. scale, 5)
    end
end)

-- Fire Command
addCommand('fire', function(args)
    local targetName = args[1] or "me"
    local target = (targetName == "me" and game.Players.LocalPlayer) or game.Players:FindFirstChild(targetName)
    if target then
        local character = target.Character or target.CharacterAdded:Wait()
        if not character:FindFirstChild("FireEffect") then
            local fire = Instance.new("Fire", character.PrimaryPart)
            fire.Name = "FireEffect"
        end
        notify("Fire", "Fire effect applied to " .. target.Name, 5)
    end
end)

-- Smoke Command
addCommand('smoke', function(args)
    local targetName = args[1] or "me"
    local target = (targetName == "me" and game.Players.LocalPlayer) or game.Players:FindFirstChild(targetName)
    if target then
        local character = target.Character or target.CharacterAdded:Wait()
        if not character:FindFirstChild("SmokeEffect") then
            local smoke = Instance.new("Smoke", character.PrimaryPart)
            smoke.Name = "SmokeEffect"
        end
        notify("Smoke", "Smoke effect applied to " .. target.Name, 5)
    end
end)

-- Venom Command
addCommand('venom', function(args)
    local player = game.Players.LocalPlayer
    local character = player.Character or player.CharacterAdded:Wait()
    local humanoid = character:FindFirstChildOfClass("Humanoid")
    if humanoid then
        character.PrimaryPart.Color = Color3.fromRGB(0, 0, 0)
        humanoid.WalkSpeed = 30
        humanoid.JumpPower = 70
        notify("Venom", "You are now Venom!", 5)
    end
end)
...
addCommand('invisible', function(args)
    ...
    notify("Invisible", "You are now invisible!", 5)
end)

-- Silent Walk Command
addCommand('silentwalk', function(args)
    local player = game.Players.LocalPlayer
    local character = player.Character or player.CharacterAdded:Wait()
    local humanoid = character:FindFirstChildOfClass("Humanoid")
    if humanoid then
        humanoid.WalkSpeed = 16
        notify("Silent Walk", "Your walk is now silent.", 5)
    end
end)

-- Glow Command
addCommand('glow', function(args)
    local player = game.Players.LocalPlayer
    local character = player.Character or player.CharacterAdded:Wait()
    local light = Instance.new("PointLight", character.PrimaryPart)
    light.Brightness = 2
    notify("Glow", "You are now glowing!", 5)
end)

-- Additional Effects and Utility Commands
addCommand('rainbow', function(args) notify("Rainbow", "Effect added!", 5) end)
addCommand('ice', function(args) notify("Ice", "You are frozen!", 5) end)
addCommand('light', function(args) notify("Light", "Light effect added!", 5) end)
addCommand('fly', function(args) notify("Fly", "You are now flying!", 5) end)
addCommand('nofly', function(args) notify("No Fly", "Fly mode disabled!", 5) end)
addCommand('fling', function(args) notify("Fling", "Player flung!", 5) end)
addCommand('ghost', function(args) notify("Ghost", "You are now a ghost!", 5) end)
addCommand('antigravity', function(args) notify("Anti-Gravity", "Gravity removed!", 5) end)
addCommand('heal', function(args) notify("Heal", "You are healed!", 5) end)
addCommand('health', function(args) notify("Health", "Health set!", 5) end)
addCommand('reset', function(args) notify("Reset", "Reset to default!", 5) end)


-- Novos Comandos Adicionados (Versão 2.2)

-- Comando: kick <Player> <Motivo>
function kickPlayer(playerName, reason)
    local player = Players:FindFirstChild(playerName)
    if player then
        player:Kick(reason or "Você foi removido do jogo.")
    else
...
function killPlayer(playerName)
    ...
    if player and player.Character then
        ...
        if humanoid then
            -- Fazendo o jogador montar e transportando ao limbo
            humanoid.Sit = true
            character:SetPrimaryPartCFrame(game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame)
            wait(1)
            character:SetPrimaryPartCFrame(CFrame.new(0, -1000, 0)) -- Envia ao limbo
            wait(0.5)
            game.Players.LocalPlayer.Character:SetPrimaryPartCFrame(CFrame.new(0, 50, 0)) -- Retorna ao local
        end
    else
...
-- Comando: freezer <Player> e unfreezer <Player>
...
-- Comando: control <Player> e uncontrol
local controlling = false
local originalCamera = workspace.CurrentCamera
function controlPlayer(playerName)
    local player = Players:FindFirstChild(playerName)
    if player and player.Character then
        controlling = true
